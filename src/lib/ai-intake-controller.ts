...
function closeRepeatedOrExpiredGaps(state: IntakeState): void {
  const expired = state.turn >= MAX_TURNS;
  for (const id of INTAKE_FACT_IDS) {
    const fact = state.facts[id];
    if ((fact.status === 'missing' || fact.status === 'partial') && (expired || fact.askedCount >= 2)) {
...
function chooseTargets(state: IntakeState, requested: unknown): IntakeFactId[] {
  ...
  const requestedIds = Array.isArray(requested)
    ...
    : [];
  const uniqueRequested = [...new Set(requestedIds)].slice(0, 2);
  ...
  return [...related, ...missing.filter((id) => !related.includes(id))].slice(0, 2);
}
...
export function advanceIntakeState(
  ...
  const targets = state.turn >= MAX_TURNS ? [] : chooseTargets(state, requestedTargets);
  for (const id of targets) state.facts[id].askedCount = Math.min(2, state.facts[id].askedCount + 1);
  state.complete = INTAKE_FACT_IDS.every((id) => state.facts[id].status !== 'missing' && state.facts[id].status !== 'partial');
  return { state, targets };
}

export function intakeProgress(state: IntakeState): { closed: number; total: number; complete: boolean } {
  return {
    closed: INTAKE_FACT_IDS.filter((id) => state.facts[id].status !== 'missing' && state.facts[id].status !== 'partial').length,
...
export function buildFinalBrief(state: IntakeState, language: IntakeLanguage): string {
  ...
  const intro = language === 'ru'
    ? 'Я собрал целостную картину задачи без дальнейшего допроса:'
    : language === 'ka'
      ? 'ზედმეტი კითხვების გარეშე ამოცანის სრული სურათი შევკრიბე:'
      : 'I have assembled a complete picture without extending the interview:';
  ...
  const closing = language === 'ru'
    ? '\n\nЭто уже достаточная основа, чтобы aiNOW предложил подход. Если я исказил какой-то факт, поправьте только его.'
    : language === 'ka'
      ? '\n\nეს უკვე საკმარისი საფუძველია, რომ aiNOW-მ მიდგომა შემოგთავაზოთ. თუ რომელიმე ფაქტი არასწორად გავიგე, მხოლოდ ის შემისწორეთ.'
      : '\n\nThis is enough for aiNOW to propose an approach. If I misread a fact, correct only that point.';
  return `${intro}\n\n${known.join('\n')}${followUp}${closing}`;
}
...
export function buildFallbackQuestion(targets: IntakeFactId[], language: IntakeLanguage): string {
  const questions = targets.map((id) => QUESTIONS[language][id]);
  const prefix = language === 'ru' ? 'Чтобы оценить задачу без догадок:' : language === 'ka' ? 'ამოცანა ვარაუდის გარეშე რომ შევაფასო, დამიზუსტეთ:' : 'To assess the task without guessing:';
...
export function fallbackSuggestions(language: IntakeLanguage, targets: IntakeFactId[]): string[] {
