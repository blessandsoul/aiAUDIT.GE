...
Branches distinguish growth, influencer attribution, customer messages, calls, ads, content, documents, web processes and operations. The product catalog also separates aiOFFICE (orders/approvals/reporting/data transfer), aiAPP (new bespoke build or integration scoping), vibeCODING (technical assessment of an existing AI-built app), aiSTAFF (a live human specialist, never a chat synonym), and aiTAXI (understood but not currently offered through Quick Audit). A direct first-turn process signal overrides only a generic model route such as `operations`; it never establishes a fact or recommendation. No fixed ten-field completion. A field is attempted at most twice, then remains an explicit gap. Users may finish early with a limited conclusion. Unknown and declined remain distinct. A supported pilot requires impact, repeatability, readiness and an inadequate simpler alternative.
...
- Real configured provider, complete synthetic scenarios: attribution → measurement_first/no product; low-reach shop → process_first/no product; repetitive clinic messages → aiCHATS pilot; own-patient booking calls → aiCALL pilot; Excel-to-warehouse transfer → aiOFFICE pilot; a new B2B request-to-CRM integration → aiAPP scoped discovery; an existing AI app with recurring failures → vibeCODING technical assessment; complex follow-up explicitly requiring a person → aiSTAFF human-service route; autonomous fleet → not available/no product.
...
- Unit coverage: question/answer contracts, provenance, refusals, anti-repetition, partial data, contradictions, readiness, false recommendations, explicit product-domain routing, aiCALL cold-list exclusion, aiTAXI non-sale and signed-state tampering. Run `npm run test:audit` using Node 24 (or a Node release supporting TypeScript stripping).
